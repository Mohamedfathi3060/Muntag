...
(function () {
  ...
  let flashSaleDeadlineDays = document.querySelector(
    ".flash-sales-section .days"
  );
  let flashSaleDeadlineHours = document.querySelector(
    ".flash-sales-section .hours"
  );
  let flashSaleDeadlineMinutes = document.querySelector(
    ".flash-sales-section .minutes"
  );
  let flashSaleDeadlineSeconds = document.querySelector(
    ".flash-sales-section .seconds"
  );
  let flashSaleProductsCardsContainer = document.querySelector(
    ".flash-sales-section .cards"
  );
  let flashSaleProductsCardLeftArrow = document.querySelector(
    ".flash-sales-section .controlicons .leftarrow"
  );
  let flashSaleProductsCardRightArrow = document.querySelector(
    ".flash-sales-section .controlicons .rightarrow"
  );
  let currentFlashSaleProductCardsPage = 1;
  let flashSaleProductsCardsStartIndex =
    (currentFlashSaleProductCardsPage - 1) * perpage;
  let flashSaleProductsCardsEndIndex =
    currentFlashSaleProductCardsPage * perpage - 1;
  let categoriesCardContainer = document.querySelector(
    ".browse-by-category-section .cards"
  );
  let categoriesLeftArrow = document.querySelector(
    ".browse-by-category-section .controlicons .leftarrow"
  );
  let categoriesRightArrow = document.querySelector(
    ".browse-by-category-section .controlicons .rightarrow"
  );
  let currentCategoriesCardsPage = 1;
  let categoriesCardsStartIndex = (currentCategoriesCardsPage - 1) * perpage;
  let categoriesCardsEndIndex = currentCategoriesCardsPage * perpage - 1;
  let exploreProductsCardsContainer = document.querySelector(
    ".explore-products-section .cards"
  );
  let exploreProductsCardLeftArrow = document.querySelector(
    ".explore-products-section .controlicons .leftarrow"
  );
  let exploreProductsCardRightArrow = document.querySelector(
    ".explore-products-section .controlicons .rightarrow"
  );
  let currentExploreProductCardsPage = 1;
  let exploreProductsCardsStartIndex =
    (currentExploreProductCardsPage - 1) * perpage;
  let exploreProductsCardsEndIndex =
    currentExploreProductCardsPage * perpage - 1;
  ...
  function nextCoverProduct() {
    if (coverProductsCount > 1)
      layoutTrackersPoints[currentCoverProduct - 1].classList.remove(
        "activetrackerpoint"
      );
    currentCoverProduct++;
    ...
    layoutTrackersPoints[currentCoverProduct - 1].classList.add(
      "activetrackerpoint"
    );
    layoutProductImg.src = coverProducts[currentCoverProduct - 1].image;
    layoutProductName.innerHTML =
      coverProducts[currentCoverProduct - 1].product_name;
    let description = coverProducts[currentCoverProduct - 1].description
      .split(" ")
      .slice(0, 7)
      .join(" ");
  ...
  function coverLayoutDisplay() {
    ...
    for (let j = 0; j < coverProductsCount; j++) {
      if (j + 1 == currentCoverProduct)
        layoutTrackersList.innerHTML += `<li class="bg-secondary rounded-5 mx-1 activetrackerpoint"></li>`;
      else
        layoutTrackersList.innerHTML += `<li class="bg-secondary rounded-5 mx-1"></li>`;
    }
    layoutTrackersPoints = document.querySelectorAll(
      ".layout .layouttrackers ul li"
    );
    setInterval(() => {
      nextCoverProduct();
    }, 5000);
  ...
  async function getFlashSaleProducts() {
    try {
      ...
      flashSaleProducts.push(...data.data);
    } catch (error) {
      console.log(error);
    }
  }

  async function nextFlashSaleProductsCard() {
    if (flashSaleProductsCardsEndIndex + perpage > exploreProducts.length) {
      currentFlashSaleProductCardsPage++;
      await getExploreProducts(currentFlashSaleProductCardsPage);
      flashSaleProductsCardsStartIndex = flashSaleProductsCardsEndIndex + 1;
      flashSaleProductsCardsEndIndex += perpage;
      flashSaleProductCardsDisplay();
    } else {
      flashSaleProductsCardsStartIndex = flashSaleProductsCardsEndIndex + 1;
      flashSaleProductsCardsEndIndex += perpage;
      flashSaleProductCardsDisplay();
  ...
  function prevFlashSaleProductsCard() {
    if (flashSaleProductsCardsStartIndex - perpage >= 0) {
      flashSaleProductsCardsEndIndex -= perpage;
      flashSaleProductsCardsStartIndex -= perpage;
      currentFlashSaleProductCardsPage--;
      flashSaleProductCardsDisplay();
    }
  }

  function flashSaleProductCardsDisplay() {
    exploreProductsCardsContainer.innerHTML = "";
    for (
      let i = exploreProductsCardsStartIndex;
      i <= exploreProductsCardsEndIndex;
      i++
    ) {
                                                                ...
                                                                <div class="card border-0 position-relative">
                                                                    ...
                                                                    <div class="cardtext my-3">
                                                                        ...
                                                                        <div class="productdetails d-flex">
                                                                            <span class="price fw-bold text-danger me-2">$${
                                                                              exploreProducts[
                                                                                i
                                                                              ]
                                                                                .price
    ...
    let exploreProductCardsIcons = document.querySelectorAll(
      ".explore-products-section .cards .col-md-3 .card .icons svg"
    );
    for (let i = 0; i < exploreProductCardsIcons.length; i++) {
      if (i % 2 == 0)
        exploreProductCardsIcons[i].addEventListener("click", function () {
  ...
  async function flashSaleProductCardsManager() {
  ...
  async function nextCategoriesCard() {
    if (categoriesCardsEndIndex + perpage > exploreProducts.length) {
      currentCategoriesCardsPage++;
      await getExploreProducts(currentCategoriesCardsPage);
      categoriesCardsStartIndex = categoriesCardsEndIndex + 1;
      categoriesCardsEndIndex += perpage;
      categoriesCardsDisplay();
    } else {
      categoriesCardsStartIndex = categoriesCardsEndIndex + 1;
      categoriesCardsEndIndex += perpage;
      categoriesCardsDisplay();
  ...
  function prevCategoriesCard() {
    if (categoriesCardsStartIndex - perpage >= 0) {
      categoriesCardsEndIndex -= perpage;
      categoriesCardsStartIndex -= perpage;
      currentCategoriesCardsPage--;
      categoriesCardsDisplay();
    }
  }

  function categoriesCardsDisplay() {
    ...
    categoryCards = document.querySelectorAll(
      ".browse-by-category-section .cards .col-md-2 .card"
    );
    if (categoriesCount > 1)
      currentCategoryCard = Math.ceil(categoryCards.length / 2);
  ...
  async function getBestSellingProducts() {
    try {
      ...
      let data = await res.json();
      bestSellingProducts.push(...data.data);
    } catch (error) {
      console.log(error);
    }
  }

  function bestSellingProductCardsDisplay() {
    exploreProductsCardsContainer.innerHTML = "";
    for (
      let i = exploreProductsCardsStartIndex;
      i <= exploreProductsCardsEndIndex;
  ...
  async function bestSellingProductCardsManager() {
    await getBestSellingProducts();
    bestSellingProductCardsDisplay();
  }

  async function getExploreProducts() {
    try {
      let res = await fetch(
        `http://${url}/api/products?page=${currentExploreProductCardsPage}&per_page=8`,
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  categoriesRightArrow.addEventListener("click", async function () {
    await nextCategoriesCard();
  });

  categoriesLeftArrow.addEventListener("click", function () {
    prevCategoriesCard();
  });
  flashSaleProductsCardRightArrow.addEventListener("click", async function () {
    await nextCategoriesCard();
  });

  flashSaleProductsCardLeftArrow.addEventListener("click", function () {
    prevCategoriesCard();
  });
